...
class InfoGcn(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, 
                 ...
                 drop_out=0, num_head=3, noise_ratio=0.1, k=0, gain=1, **kwargs):
    ...
    def forward(self, x):
        N, C, T, V, M = x.size()
        x = rearrange(x, 'n c t v m -> (n m t) v c', m=M, v=V).contiguous().float()
        x = self.A_vector.to(x.device).expand(N*M*T, -1, -1).float() @ x.float()
